...
bool Logger::FormatsNextPendingLog() {
	...
	if (Logger::Get().d_queue.TryRemove(msg) == false) {
		return false;
	}
...
void Logger::ScheduleLogFormatting() {
	Scheduler::Get().Schedule(
	    1000,
	    FormatsNextPendingLog,
	    {.Priority = SCHEDULER_LOW_PRIORITY, .Name = "log/output"}
	);
}
